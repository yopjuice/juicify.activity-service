import { STATS_SERVICE_NAME, StatsServiceClient } from '@juice11-micro/contracts';
...
@Injectable()
export class StatsGrpc implements OnModuleInit {
  private readonly logger = new Logger(StatsGrpc.name);
  private rawService: StatsServiceClient;
  
  public client: GrpcToPromise<StatsServiceClient>; 

  constructor(
    @Inject('STATS_INTERNAL_PROXY') private readonly grpcClient: ClientGrpc,
  ) {}

  onModuleInit() {
    // Original gRPC service from Nest
    this.rawService = this.grpcClient.getService<any>(STATS_SERVICE_NAME);
