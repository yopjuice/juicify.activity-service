import { FAVORITES_SERVICE_NAME, FavoritesServiceClient } from '@juice11-micro/contracts';
...
@Injectable()
export class FavoriteGrpc implements OnModuleInit {
  private readonly logger = new Logger(FavoriteGrpc.name);
  private rawService: FavoritesServiceClient;
  
  public client: GrpcToPromise<FavoritesServiceClient>; 

  constructor(
    @Inject('FAVORITE_INTERNAL_PROXY') private readonly grpcClient: ClientGrpc,
  ) {}

  onModuleInit() {
    // Original gRPC service from Nest
    this.rawService = this.grpcClient.getService<any>(FAVORITES_SERVICE_NAME);
