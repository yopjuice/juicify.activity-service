...
@Injectable()
export class InteractionGrpc implements OnModuleInit {
  private readonly logger = new Logger(InteractionGrpc.name);
  private rawService: InteractionServiceClient;
  
  public client: GrpcToPromise<InteractionServiceClient>; 

  constructor(
    @Inject('INTERACTION_INTERNAL_PROXY') private readonly grpcClient: ClientGrpc,
  ) {}

  onModuleInit() {
    // Original gRPC service from Nest
    this.rawService = this.grpcClient.getService<any>('InteractionService');
